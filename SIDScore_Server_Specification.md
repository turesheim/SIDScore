...
0x19 EXPORT_SOURCE      client -> server
...
0x29 EXPORT_RESULT      server -> client
...
`EXPORT_SOURCE` exports SIDScore source text sent directly over SRAP. It is the
preferred IDE export command because it can export the current editor buffer
without requiring a save first. `sourcePath` is used as the base path for
relative imports and diagnostics. `outputPath` is a local filesystem path that
...
semantics; export uses the resolved score source.

On success, the server sends `EXPORT_RESULT`. On failure, it sends `ERROR` with
